...
unsigned c_Rank::getNumBankGroups() const { return m_bankGroupPtrs.size(); }

c_Channel *c_Rank::getChannelPtr() const { return m_channelPtr; }
...
std::vector<c_BankInfo *> &c_Rank::getBankPtrs() {
  ...
  if (m_allBankPtrs.empty()) {
    for (auto &l_bankGroupPtr : m_bankGroupPtrs) {
      ...
      m_allBankPtrs.insert(m_allBankPtrs.end(), l_entryPtrs.begin(),
                           l_entryPtrs.end());
    }
  }
  ...
  for (auto &l_bankGroupPtr : m_bankGroupPtrs) {
    ...
    for (auto &l_bankPtr : l_bankGroupPtr->getBankPtrs()) {
      ...
      case e_BankCommandType::ACT: {
        SimTime_t l_nextCycle =
            std::max(l_bankPtr->getNextCommandCycle(e_BankCommandType::ACT),
      ...
      case e_BankCommandType::READA: {
        ...
        SimTime_t l_nextCycle = std::max(
            std::max(l_bankPtr->getNextCommandCycle(e_BankCommandType::READ),
                     l_bankPtr->getNextCommandCycle(e_BankCommandType::READA)),
            l_time +
                std::max(m_bankParams->at("nCCD_S"), m_bankParams->at("nBL")));

        l_bankPtr->setNextCommandCycle(e_BankCommandType::READ, l_nextCycle);
        l_bankPtr->setNextCommandCycle(e_BankCommandType::READA, l_nextCycle);

        // timing for the next WRITE or WRITEA command
        l_nextCycle = std::max(
            std::max(l_bankPtr->getNextCommandCycle(e_BankCommandType::WRITE),
                     l_bankPtr->getNextCommandCycle(e_BankCommandType::WRITEA)),
            l_time + m_bankParams->at("nCL") + m_bankParams->at("nBL") +
                m_bankParams->at("nRTW") - m_bankParams->at("nCWL"));

        l_bankPtr->setNextCommandCycle(e_BankCommandType::WRITE, l_nextCycle);
        l_bankPtr->setNextCommandCycle(e_BankCommandType::WRITEA, l_nextCycle);
