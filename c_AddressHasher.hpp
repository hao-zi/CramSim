...
public:
  SST_ELI_REGISTER_SUBCOMPONENT_API(SST::CramSim::c_AddressHasher, Output *,
                                    unsigned, unsigned, unsigned, unsigned,
                                    unsigned, unsigned, unsigned)

  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      c_AddressHasher, "CramSim", "c_AddressHasher",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "Hashes addresses based on config parameters",
      SST::CramSim::c_AddressHasher)

  SST_ELI_DOCUMENT_PARAMS({"numBytesPerTransaction",
                           "Number of bytes retrieved for every transaction",
                           "1"},
                          {"strAddressMapStr",
                           "String defining the address mapping scheme",
                           "_r_l_b_R_B_h_"}, )
  ...
  // Below is for calling in generic locations to obtain a pointer to the
  // singleton instance
  c_AddressHasher(ComponentId_t id, Params &params, Output *out,
                  unsigned channels, unsigned ranks, unsigned bankGroups,
                  unsigned banks, unsigned rows, unsigned cols,
                  unsigned pChannels);
  void build(Params &params); // Temporary until subcomponent api transition is
                              // complete

  static c_AddressHasher *getInstance();

  static c_AddressHasher *
  getInstance(Params &x_params); // This reads the parameters and constructs the
                                 // hash function
...
private:
  ...
  std::map<std::string, uint>
      m_structureSizes; // Used for checking that params agree

  // regex replacement stuff
  void parsePattern(std::string *x_inStr,
                    std::pair<std::string, uint> *x_outPair);
