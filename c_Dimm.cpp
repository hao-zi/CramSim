...
c_Dimm::c_Dimm(SST::ComponentId_t x_id, SST::Params &x_params)
  ...
  m_ctrlLink = configureLink(
      "ctrlLink",
      new Event::Handler<c_Dimm>(this, &c_Dimm::handleInCmdUnitReqPtrEvent));
  ...
  k_numPChannelsPerChannel =
      (uint32_t)x_params.find<uint32_t>("numPChannelsPerChannel", 1, l_found);
  if (!l_found) {
  ...
  k_numRanksPerChannel =
      (uint32_t)x_params.find<uint32_t>("numRanksPerChannel", 100, l_found);
  if (!l_found) {
  ...
  k_numBankGroupsPerRank =
      (uint32_t)x_params.find<uint32_t>("numBankGroupsPerRank", 100, l_found);
  if (!l_found) {
  ...
  k_numBanksPerBankGroup =
      (uint32_t)x_params.find<uint32_t>("numBanksPerBankGroup", 100, l_found);
  if (!l_found) {
  ...
  if (k_boolPowerCalc) {
    int l_numRank =
        k_numRanksPerChannel * k_numChannels * k_numPChannelsPerChannel;
    ...
    if (!l_found) {
      std::cout << "[c_Dimm] Power calculation is enabled, but IDD0 value is "
                   "missing... exiting"
                << std::endl;
      exit(-1);
    }
    ...
    if (!l_found) {
      std::cout << "[c_Dimm] Power calculation is enabled, but IDD2N value is "
                   "missing... exiting"
                << std::endl;
      exit(-1);
    }
    ...
    if (!l_found) {
      std::cout << "[c_Dimm] Power calculation is enabled, but IDD2P value is "
                   "missing... exiting"
                << std::endl;
      exit(-1);
    }
    ...
    if (!l_found) {
      std::cout << "[c_Dimm] Power calculation is enabled, but IDD3N value is "
                   "missing... exiting"
                << std::endl;
      exit(-1);
    }
    ...
    if (!l_found) {
      std::cout << "[c_Dimm] Power calculation is enabled, but IDD4R value is "
                   "missing... exiting"
                << std::endl;
      exit(-1);
    }
    ...
    if (!l_found) {
      std::cout << "[c_Dimm] Power calculation is enabled, but IDD4W value is "
                   "missing... exiting"
                << std::endl;
      exit(-1);
    }
    ...
    if (!l_found) {
      std::cout << "[c_Dimm] Power calculation is enabled, but IDD5 value is "
                   "missing... exiting"
                << std::endl;
      exit(-1);
    }
    ...
    if (!l_found) {
      std::cout << "[c_Dimm] Power calculation is enabled, but nRAS value is "
                   "missing ... exiting"
                << std::endl;
      exit(-1);
    }
    ...
    if (!l_found) {
      std::cout << "[c_Dimm] Power calculation is enabled, but nRP value is "
                   "missing ... exiting"
                << std::endl;
      exit(-1);
    }
    ...
    if (!l_found) {
      std::cout << "[c_Dimm] Power calculation is enabled, but nRFC value is "
                   "missing ... exiting"
                << std::endl;
      exit(-1);
    }
    ...
    if (!l_found) {
      std::cout << "[c_Dimm] Power calculation is enabled, but nBL value is "
                   "missing ... exiting"
                << std::endl;
      exit(-1);
    }
    ...
    if (!l_found) {
      std::cout << "[c_Dimm] Power calculation is enabled, but VDD value is "
                   "missing ... exiting"
                << std::endl;
      exit(-1);
    }
    ...
    if (!l_found) {
      std::cout << "[c_Dimm] Power calculation is enabled, but numDevices "
                   "value is missing ... exiting"
                << std::endl;
      exit(-1);
    }
  }

  Statistic<uint64_t> *l_totalRowHits =
      registerStatistic<uint64_t>("totalRowHits");
...
void c_Dimm::updateDynamicEnergy(c_BankCommand *x_bankCommandPtr) {
  ...
  case e_BankCommandType::ACT:
    l_energy = ((k_IDD0 * (k_nRAS + k_nRP)) -
                ((k_IDD3N * k_nRAS) + (k_IDD2N * (k_nRP)))) *
               k_VDD * k_numDevices; // active and precharge
  ...
  if (x_bankCommandPtr->getBankIdVec().size() > 0) {
    for (auto &l_bankid : x_bankCommandPtr->getBankIdVec()) {
      c_BankCommand *l_cmd = new c_BankCommand(
          x_bankCommandPtr->getSeqNum(), x_bankCommandPtr->getCommandMnemonic(),
...
void c_Dimm::sendResponse() {
  ...
  while (!m_cmdResQ.empty()) {
    ...
    m_cmdResQ.erase(
        std::remove(m_cmdResQ.begin(), m_cmdResQ.end(), m_cmdResQ.front()),
        m_cmdResQ.end());
...
void c_Dimm::finish() {
  ...
  if (k_boolPowerCalc) {
    ...
    double l_totalPower = l_actprePower + l_readPower + l_writePower +
                          l_refreshPower + l_backgroundPower;
    std::cout << std::endl;
    std::cout << " 2. Power Consumption" << std::endl;
    std::cout << "  - Total Power (mW) : " << l_totalPower << std::endl;
    std::cout << "  - Active/Precharge Power (mW) : " << l_actprePower << "("
              << l_actprePower / l_totalPower * 100 << "%)" << std::endl;
    std::cout << "  - Read Power (mW) : " << l_readPower << "("
              << l_readPower / l_totalPower * 100 << "%)" << std::endl;
    std::cout << "  - Write Power (mW) : " << l_writePower << "("
              << l_writePower / l_totalPower * 100 << "%)" << std::endl;
    std::cout << "  - Refresh Power (mW) : " << l_refreshPower << "("
              << l_refreshPower / l_totalPower * 100 << "%)" << std::endl;
    std::cout << "  - Background Power (mW) : " << l_backgroundPower << "("
              << l_backgroundPower / l_totalPower * 100 << "%)" << std::endl;
  }
  std::cout << "==============================================================="
               "======================\n";
}
