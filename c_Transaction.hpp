...
private:
  ...
  bool m_processed; //<! flag that is set when this transaction is split into
                    //commands

  // std::list<c_BankCommand*> m_cmdPtrList; //<! list of c_BankCommand
  // shared_ptrs that compose this c_Transaction
  std::list<ulong> m_cmdSeqNumList; //<! list of c_BankCommand Sequence numbers
                                    //that compose this c_Transaction
  c_HashedAddress m_hashedAddr;
  bool m_hasHashedAddr;

public:
  //  friend std::ostream& operator<< (std::ostream& x_stream, const
  //  c_Transaction& x_transaction);
  ...
  void setResponseReady(); //<! sets the flag that this transaction has received
                           //its response.
  bool isResponseReady();  //<! returns the flag that this transaction has
                           //received its response.

  void setWaitingCommands(const unsigned x_numWaitingCommands);
  unsigned getWaitingCommands() const;

  bool matchesCmdSeqNum(ulong x_seqNum); //<! returns true if this transaction
                                         //matches a command with x_seqNum
  ...
  void print() const;
  void print(SST::Output *x_output, std::string x_prefix,
             SimTime_t x_cycle) const;

  const c_HashedAddress &getHashedAddress() const { return (m_hashedAddr); }
  void setHashedAddress(c_HashedAddress &x_hashedAddr) {
    m_hashedAddr = x_hashedAddr;
    m_hasHashedAddr = true;
  }
  bool hasHashedAddress() { return m_hasHashedAddr; }

  bool isRead() { return m_txnMnemonic == e_TransactionType ::READ; }

  bool isWrite() { return m_txnMnemonic == e_TransactionType ::WRITE; }
