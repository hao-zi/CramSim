...
unsigned c_BankGroup::getNumBanks() const { return m_bankPtrs.size(); }

unsigned c_BankGroup::getBankGroupId() const { return m_bankGroupId; }

std::vector<c_BankInfo *> c_BankGroup::getBankPtrs() const {
  return (m_bankPtrs);
}

c_Rank *c_BankGroup::getRankPtr() const { return m_rankPtr; }
  ...
  for (std::vector<c_BankInfo *>::iterator l_iter = m_bankPtrs.begin();
    ...
    case e_BankCommandType::READA: {
      ...
      SimTime_t l_nextCycle = std::max(
          ...
          l_time +
              std::max(m_bankParams->at("nCCD_L"), m_bankParams->at("nBL")));
