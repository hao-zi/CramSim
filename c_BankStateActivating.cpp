...
void c_BankStateActivating::handleCommand(c_BankInfo *x_bank,
                                          c_BankCommand *x_bankCommandPtr,
                                          SimTime_t x_cycle) {
  std::cout << __PRETTY_FUNCTION__
            << " ERROR: should not receive a command in this state. This is a "
               "transitory state."
            << std::endl;
}
...
void c_BankStateActivating::clockTic(c_BankInfo *x_bank, SimTime_t x_cycle) {
  ...
  if (0 < m_timer) {
    ...
    auto l_p =
        new c_BankStateActive(m_bankParams); // create pointer to the next state
    assert(e_BankCommandType::ACT ==
           m_prevCommandPtr
               ->getCommandMnemonic()); // only cmd allowed to flow through to
                                        // BankStateActive is ACT
    l_p->enter(x_bank, this, m_prevCommandPtr, x_cycle);
  }
}

void c_BankStateActivating::enter(c_BankInfo *x_bank, c_BankState *x_prevState,
...
bool c_BankStateActivating::isCommandAllowed(c_BankCommand *x_cmdPtr,
  ...
  // Cmd must be of an allowed type and BankState cannot already be processing
  // another cmd
  for (std::list<e_BankCommandType>::iterator l_iter =
           m_allowedCommands.begin();
       l_iter != m_allowedCommands.end(); ++l_iter) {
