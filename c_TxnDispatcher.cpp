...
c_TxnDispatcher::c_TxnDispatcher(ComponentId_t x_id, Params &params)
    : Component(x_id) {
  //*------ get parameters ----*//
  bool l_found = false;
  ...
  dbg.init("[TxnDispatcher]", params.find<int>("debug_level", 0), 0,
           (Output::output_location_t)params.find<int>("debug_location",
                                                       Output::STDOUT));

  k_numLanes = (uint32_t)params.find<uint32_t>("numLanes", 1, l_found);
  if (!l_found) {
    cout << "numLanes param value is missing... exiting" << endl;
    exit(-1);
  }
  assert(k_numLanes > 0);

  string l_laneIdxString =
      (string)params.find<string>("laneIdxPos", "13:12", l_found);
  if (k_numLanes > 1) {
    if (!l_found) {
      cout << "the bit position of lane index is not specified... it should be "
              "\"end:start\""
           << endl;
      exit(-1);
    } else {
      ...
      if (strings.size() != 2) {
        ...
        if (m_laneIdxEnd < m_laneIdxStart) {
          cout << "landIdxPos error!!"
               ...
               << "Start position: " << m_laneIdxStart << endl;
          exit(-1);
        }
      }
    }
  }
  std::string l_clockFreqStr =
      (std::string)params.find<std::string>("ClockFreq", "1GHz", l_found);

  // set our clock
  registerClock(l_clockFreqStr, new Clock::Handler<c_TxnDispatcher>(
                                    this, &c_TxnDispatcher::clockTic));

  //---- configure link ----//
  m_txnGenLink =
      configureLink("txnGen", new Event::Handler<c_TxnDispatcher>(
                                  this, &c_TxnDispatcher::handleTxnGenEvent));
  if (!m_txnGenLink) {
  ...
  for (int i = 0; i < k_numLanes; i++) {
    ...
    Link *l_link =
        configureLink(l_linkName, new Event::Handler<c_TxnDispatcher>(
                                      this, &c_TxnDispatcher::handleCtrlEvent));
...
c_TxnDispatcher::c_TxnDispatcher() : Component(-1) {
...
void c_TxnDispatcher::sendRequest(c_TxnReqEvent *x_newReq) {
  ...
  dbg.verbose(CALL_INFO, 1, 0,
              " Cycle:%lld, LaneIdxPosition:[%d:%d] Addr: 0x%llx LaneIdx: %d\n",
