...
c_DeviceDriver::c_DeviceDriver(ComponentId_t id, Params &params, Output *out,
                               std::function<void(c_BankCommand *)> sendFunc)
    : SubComponent(id), output(out), m_sendCmdFunc(sendFunc) {
  build(params);
}
...
void c_DeviceDriver::build(Params &params) {
  ...
  k_useDualCommandBus =
      (uint32_t)params.find<uint32_t>("boolDualCommandBus", 0, l_found);
  if (!l_found) {
    output->output("boolDualCommandBus value is missing... disabled\n");
  }

  k_multiCycleACT =
      (uint32_t)params.find<uint32_t>("boolMultiCycleACT", 0, l_found);
  if (!l_found) {
  ...
  k_numPChannelsPerChannel =
      (uint32_t)params.find<uint32_t>("numPChannelsPerChannel", 1, l_found);
  if (!l_found) {
    output->output("numPChannelsPerChannel value is missing... disabled\n");
  }

  k_numRanksPerChannel =
      (uint32_t)params.find<uint32_t>("numRanksPerChannel", 2, l_found);
  if (!l_found) {
    output->fatal(CALL_INFO, -1,
                  "numRanksPerChannel value is missing... exiting\n");
  }

  k_numBankGroupsPerRank =
      (uint32_t)params.find<uint32_t>("numBankGroupsPerRank", 100, l_found);
  if (!l_found) {
    output->fatal(CALL_INFO, -1,
                  "numBankGroupsPerRank value is missing... exiting\n");
  }

  k_numBanksPerBankGroup =
      (uint32_t)params.find<uint32_t>("numBanksPerBankGroup", 100, l_found);
  if (!l_found) {
    output->fatal(CALL_INFO, -1,
                  "numBanksPerBankGroup value is missing... exiting\n");
  }

  k_numColsPerBank =
      (uint32_t)params.find<uint32_t>("numColsPerBank", 100, l_found);
  if (!l_found) {
    output->fatal(CALL_INFO, -1,
                  "numColsPerBank value is missing... exiting\n");
  }

  k_numRowsPerBank =
      (uint32_t)params.find<uint32_t>("numRowsPerBank", 100, l_found);
  if (!l_found) {
    output->fatal(CALL_INFO, -1,
                  "numRowsPerBank value is missing... exiting\n");
  }
  ...
  k_useSBRefresh =
      (uint32_t)params.find<uint32_t>("boolUseSBRefresh", 0, l_found);
  if (!l_found) {
    output->output("boolUseSBRefresh (single bank refresh) param value is "
                   "missing... disabled\n");
  }
  ...
  m_bankParams["nRRD_L"] =
      (uint32_t)params.find<uint32_t>("nRRD_L", 6, l_found);
  if (!l_found) {
    output->fatal(CALL_INFO, -1, "nRRD_L value is missing ... exiting\n");
  }

  m_bankParams["nRRD_S"] =
      (uint32_t)params.find<uint32_t>("nRRD_S", 4, l_found);
  if (!l_found) {
  ...
  m_bankParams["nCCD_L"] =
      (uint32_t)params.find<uint32_t>("nCCD_L", 5, l_found);
  if (!l_found) {
    output->fatal(CALL_INFO, -1, "nCCD_L value is missing ... exiting\n");
  }

  m_bankParams["nCCD_L_WR"] =
      (uint32_t)params.find<uint32_t>("nCCD_L_WR", 1, l_found);
  if (!l_found) {
    output->fatal(CALL_INFO, -1, "nCCD_L_WR value is missing ... exiting\n");
  }

  m_bankParams["nCCD_S"] =
      (uint32_t)params.find<uint32_t>("nCCD_S", 4, l_found);
  if (!l_found) {
  ...
  m_bankParams["nWTR_L"] =
      (uint32_t)params.find<uint32_t>("nWTR_L", 9, l_found);
  if (!l_found) {
    output->fatal(CALL_INFO, -1, "nWTR_L value is missing ... exiting\n");
  }

  m_bankParams["nWTR_S"] =
      (uint32_t)params.find<uint32_t>("nWTR_S", 3, l_found);
  if (!l_found) {
  ...
  m_bankParams["nREFI"] =
      (uint32_t)params.find<uint32_t>("nREFI", 9360, l_found);
  if (!l_found) {
  ...
  for (int l_i = 0; l_i != m_numBanks; ++l_i) {
    c_BankInfo *l_entry =
        new c_BankInfo(&m_bankParams, l_i); // auto init state to IDLE
  ...
  k_printCmdTrace =
      (uint32_t)params.find<uint32_t>("boolPrintCmdTrace", 0, l_found);

  k_cmdTraceFileName =
      (std::string)params.find<std::string>("strCmdTraceFile", "-", l_found);
  ...
  if (k_printCmdTrace) {
    if (k_cmdTraceFileName.compare("-") == 0) { // set output to std::cout
      ...
      std::cout << "Setting cmd trace output to " << k_cmdTraceFileName
                << std::endl;
      m_cmdTraceOFStream.open(k_cmdTraceFileName);
      if (m_cmdTraceOFStream) {
        m_cmdTraceStreamBuf = m_cmdTraceOFStream.rdbuf();
      } else {
        std::cerr << "Failed to open cmd trace output file "
                  << k_cmdTraceFileName << ", redirecting to stdout";
        m_cmdTraceStreamBuf = std::cout.rdbuf();
      }
    }
...
void c_DeviceDriver::update(SimTime_t simCycle) {
  ...
  for (int l_rankNum = 0; l_rankNum < m_numRanks; l_rankNum++) {
    m_cmdACTFAWtrackers[l_rankNum].push_back(m_isACTIssued[l_rankNum]
...
bool c_DeviceDriver::isCmdAllowed(c_BankCommand *x_bankCommandPtr) {
  ...
  // insert active command if the target row is closed (this case can happen due
  // to refresh)
  if (x_bankCommandPtr->isColCommand() &&
    ...
    c_BankCommand *l_newCmd =
        new c_BankCommand(0, e_BankCommandType::ACT, l_addr, *l_hashedAddr);
  ...
  //	if ((e_BankCommandType::ACT ==
  //((x_bankCommandPtr))->getCommandMnemonic()) && (l_cmdACTIssuedInFAW >= 4))
  //		return false;

  // check inflight Write commands to the same address
  // block: READ after WRITE to the same address
  // block: WRITE after WRITE to the same address. Processor should make sure
  // that the older WRITE is annulled but we will block the younger here.
  // todo: remove?
...
void c_DeviceDriver::sendRequest() {
  ...
  for (auto l_cmdPtrItr = m_inputQ.begin(); l_cmdPtrItr != m_inputQ.end();) {
    ...
    if ((e_BankCommandType::WRITE == (l_cmdPtr)->getCommandMnemonic()) ||
        (e_BankCommandType::WRITEA == (l_cmdPtr)->getCommandMnemonic())) {
    ...
    if (l_proceed) {
      ...
      if (isCommandBusAvailable(
              l_cmdPtr)) { // check if the command bus is available
        ...
        if (sendCommand((l_cmdPtr), l_bank)) {
          ...
          bool l_isACT =
              (e_BankCommandType::ACT == ((l_cmdPtr))->getCommandMnemonic());
          if (l_isACT) {
...
bool c_DeviceDriver::sendRefresh(unsigned x_requester) {
  ...
  if (isCommandBusAvailable(l_cmdPtr)) { // check if the command bus is
                                         // available
    if (l_cmdPtr->getBankIdVec().size() > 0) {
      for (auto &l_bankid : l_cmdPtr->getBankIdVec()) {
        c_BankInfo *l_bank = m_banks.at(l_bankid);
        c_BankCommand *l_cmd = new c_BankCommand(
            l_bankid, l_cmdPtr->getCommandMnemonic(), l_cmdPtr->getAddress(),
            *(l_cmdPtr->getHashedAddress()), l_cmdPtr->getBankIdVec());
 ...
 *
 * @param l_cmdPtr
 * @return "true" if the command bus required for the input command is
 * available.
 */
...
bool c_DeviceDriver::occupyCommandBus(c_BankCommand *l_cmdPtr) {
  ...
  if (l_cmdPtr->getCommandMnemonic() == e_BankCommandType::ACT &&
      k_multiCycleACT)
  ...
  if (x_bank->isCommandAllowed(x_bankCommandPtr, l_time)) {
    if (k_printCmdTrace) {
      ...
      if (x_bankCommandPtr->isRefreshType()) {
        (*m_cmdTraceStream) << "@" << std::dec << m_simCycle << " "
                            << (x_bankCommandPtr)->getCommandString() << " "
                            << std::dec << (x_bankCommandPtr)->getSeqNum()
                            << " " << std::dec << l_bankId << std::endl;
      } else {
        (*m_cmdTraceStream)
            << "@" << std::dec << m_simCycle << " "
            << (x_bankCommandPtr)->getCommandString() << " " << std::dec
            << (x_bankCommandPtr)->getSeqNum() << " 0x" << std::hex
            << (x_bankCommandPtr)->getAddress() << " " << std::dec
            ...
            << "\t" << std::dec << x_bankCommandPtr->getBankId() << std::endl;
      }
    }

#ifdef __SST_DEBUG_OUTPUT__
    output->verbose(CALL_INFO, 1, 0,
                    "Cycle:%lld Cmd:%s CH:%d PCH:%d Rank:%d BG:%d B:%d Row:%d "
                    "Col:%d BankId:%d CmdSeq:%lld\n",
                    m_simCycle, x_bankCommandPtr->getCommandString().c_str(),
                    x_bankCommandPtr->getHashedAddress()->getChannel(),
...
void c_DeviceDriver::createRefreshCmds(unsigned x_rankID) {
  ...
  if (k_useSBRefresh) // per-bank refresh (single bank refresh)
  {
    ...
    l_refreshRequester =
        m_ranks[x_rankID]->getBankPtrs(); // all bank will be refreshed

  // get channel id --> used for the command bus arbitration
  unsigned l_ch = l_refreshRequester.front()
                      ->getBankGroupPtr()
                      ->getRankPtr()
                      ->getChannelPtr()
                      ->getChannelId();
  ...
  c_HashedAddress *l_hashedAddress =
      new c_HashedAddress(l_ch, l_pch, 0, 0, 0, 0, 0, 0);
  ...
  if (!l_bankVec.empty()) {
    ...
    m_refreshCmdQ[x_rankID].push_back(new c_BankCommand(
        0, e_BankCommandType::PRE, 0, *l_hashedAddress, l_bankVec));
  ...
  if (!l_bankVec.empty()) {
    ...
    m_refreshCmdQ[x_rankID].push_back(new c_BankCommand(
        0, e_BankCommandType::REF, 0, *l_hashedAddress, l_bankVec));
    delete l_hashedAddress;
  }
  ...
  for (auto &l_bank : l_refreshRequester) {
    for (auto &l_cmd : m_inputQ) {
      if (l_bank->getBankId() == l_cmd->getBankId()) {
        // add active commands if there is a column command going to the bank
        // closed by the refresh operation.
        if (l_cmd->isColCommand()) {
          ulong l_addr = l_cmd->getAddress();
          const c_HashedAddress *l_hashedAddr = l_cmd->getHashedAddress();
          c_BankCommand *l_newCmd = new c_BankCommand(0, e_BankCommandType::ACT,
                                                      l_addr, *l_hashedAddr);
          m_inputQ.push_front(l_newCmd);

          break;
        } else // stop if there is a row command (precharge or active) followed
               // by a column command
          break;
      }
    }
