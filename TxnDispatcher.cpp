...
c_TxnDispatcher::c_TxnDispatcher(ComponentId_t id, Params &params) {
  ...
  if (!l_found) {
    std::cout << "numLanes param value is missing... exiting" << std::endl;
    exit(-1);
  }
  std::string l_laneIdxString = (std::string)params.find<std::string>(
      "laneIdxPosition", "[13:12]", l_found);
  if (!l_found) {
    std::cout << "the bit position of lane index is not specified... it should "
                 "be [end:start]"
              << std::endl;
    exit(-1);
  } else {
    l_laneIdxString.erase(
        remove(l_laneIdxString.begin(), l_laneIdxstring.end(), '['),
        l_laneIdxString.end());
    l_laneIdxString.erase(
        remove(l_laneIdxString.begin(), l_laneIdxstring.end(), ']'),
        l_laneIdxString.end());
  ...
  for (int i = 0; i < numLanes; i++) {
    ...
    if (l_link) {
      m_outLaneLinks.push_back(l_link,
                               new Event::Handler<c_TxnDispatcher>(
