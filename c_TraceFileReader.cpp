...
c_TraceFileReader::c_TraceFileReader(SST::ComponentId_t x_id,
                                     SST::Params &x_params)
  ...
  if (!l_found) {
    ...
    std::cout << "TraceFileReader: tracefile name is" << m_traceFileName
              << std::endl;
  }
  ...
  // get trace file type
  std::string l_traceFileType =
      x_params.find<std::string>("traceFileType", "DEFAULT", l_found);
  if (!l_found) {
    std::cout << "TraceFileReader: traceFile type is missing... default "
                 "(DRAMSim2 type)"
              << std::endl;
  }
...
void c_TraceFileReader::createTxn() {
  ...
  while (m_txnReqQ.size() < k_numTxnPerCycle) {
    ...
    if (std::getline(*m_traceFileStream, l_line)) {
      ...
      for (Tokenizer<>::iterator l_iter = l_tok.begin(); l_iter != l_tok.end();
           ++l_iter) {
        ...
        if (m_traceType == e_TracefileType::DEFAULT) {
          ...
          case 2:
            ...
            break;
          default:
            std::cout << "TraceFileReader Should not be in this stage of "
                         "switch statement"
                      << std::endl;
            exit(-1);
            break;
      ...
      c_Transaction *l_txn =
          new c_Transaction(m_seqNum, l_txnType, l_txnAddress, 1);
      std::pair<c_Transaction *, unsigned> l_entry =
          std::make_pair(l_txn, l_txnInterval);
