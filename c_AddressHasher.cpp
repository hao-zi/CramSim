...
c_AddressHasher::c_AddressHasher(ComponentId_t id, Params &params, Output *out,
                                 unsigned channels, unsigned ranks,
                                 unsigned bankGroups, unsigned banks,
                                 unsigned rows, unsigned cols,
                                 unsigned pChannels)
    : SubComponent(id), output(out), k_pNumChannels(channels),
      k_pNumRanks(ranks), k_pNumBankGroups(bankGroups), k_pNumBanks(banks),
      k_pNumRows(rows), k_pNumCols(cols), k_pNumPseudoChannels(pChannels) {
  build(params);
}
...
void c_AddressHasher::build(Params &params) {
  ...
  bool l_found = false;
  k_addressMapStr =
      (string)params.find<string>("strAddressMapStr", "_r_l_b_R_B_h_", l_found);
  output->output("Address map string: %s\n", k_addressMapStr.c_str());

  string l_mapCopy = k_addressMapStr;

  // first, remove all _'s
  l_mapCopy.erase(remove(l_mapCopy.begin(), l_mapCopy.end(), '_'),
                  l_mapCopy.end());
  // remove trailing newline
  l_mapCopy.erase(remove(l_mapCopy.begin(), l_mapCopy.end(), '\n'),
                  l_mapCopy.end());

  // this is replacement code for the above, which is generally safer than my
  // implemention of parsePattern below
  uint l_curPos = 0;
  ...
  k_pBurstSize =
      (uint32_t)params.find<uint32_t>("numBytesPerTransaction", 1, l_found);
  ...
  if (l_allSimple) { // if simple address detected, reset the bitPositions
                     // structureSizes
    // reset bitPositions
    ...
    l_cfgBits["c"] =
        (uint)log2(k_pNumPseudoChannels); // set the number of address bits
                                          // assigned to pseudo channel.
  ...
  if (l_it == m_structureSizes.end()) {   // if not found
    if (k_pNumChannels > 1) {
      output->fatal(
          CALL_INFO, -1,
          "%s, Number of Channels (%u) is greater than 1, but no Channels were "
          "specified (C) in the address map! Aborting!\n",
          getName().c_str(), k_pNumChannels);
    }
  } else { // found in map
    auto l_aNumChannels = (1 << l_it->second);
    if (l_aNumChannels > k_pNumChannels) {
      output->output("%s, Warning!: Number of address map channels is larger "
                     "than numChannelsPerDimm.\nSome channels will be unused\n",
                     getName().c_str());
    }
    if (l_aNumChannels < k_pNumChannels) { // some addresses have nowhere to go
      output->fatal(CALL_INFO, -1,
                    "%s, Error!: Number of address map channels is smaller "
                    "than numChannelsPerDimm. Aborting!",
                    getName().c_str());
    }
  } // else found in map
  ...
  if (l_it == m_structureSizes.end()) { // if not found
    if (k_pNumPseudoChannels > 1) {
      ...
      output->fatal(
          CALL_INFO, -1,
          "%s, Number of Pseudo Channels (%u) is greater than 1, but no "
          "Channels were specified (c) in the address map! Aborting!\n",
    ...
    if (l_aNumPChannels > k_pNumPseudoChannels) {
      output->output("%s, Warning!: Number of address map channels is larger "
                     "than numPseudoChannels.\n"
                     "Some channels will be unused\n\n",
                     getName().c_str());
    }
    if (l_aNumPChannels <
        k_pNumPseudoChannels) { // some addresses have nowhere to go
      output->fatal(CALL_INFO, -1,
                    "%s, Error!: Number of address map channels is smaller "
                    "than numPseudoChannels. Aborting!\n",
                    getName().c_str());
    }
  } // else found in map
  ...
  if (l_it == m_structureSizes.end()) { // if not found
    if (k_pNumRanks > 1) {
      output->fatal(CALL_INFO, -1,
                    "%s, Number of Ranks (%u) is greater than 1, but no Ranks "
                    "were specified (R) en the address map! Aborting!\n",
                    getName().c_str(), k_pNumRanks);
    }
  } else { // found in map
    auto l_aNumRanks = (1 << l_it->second);
    if (l_aNumRanks > k_pNumRanks) {
      output->output("%s, Warning!: Number of address map Ranks is larger than "
                     "numRanksPerChannel.\n"
    ...
    if (l_aNumRanks < k_pNumRanks) { // some addresses have nowhere to go
      output->fatal(CALL_INFO, -1,
                    "%s, Error!: Number of address map Ranks is smaller than "
                    "numRanksPerChannel. Aborting!\n",
                    getName().c_str());
    }
  } // else found in map
  ...
  if (l_it == m_structureSizes.end()) { // if not found
    if (k_pNumBankGroups > 1) {
      output->fatal(
          CALL_INFO, -1,
          "%s, Number of BankGroups (%u) is greater than 1, but no BankGroups "
          "were specified (B) in the address map! Aborting!\n",
          getName().c_str(), k_pNumBankGroups);
    }
  } else { // found in map
    auto l_aNumBankGroups = (1 << l_it->second);
    if (l_aNumBankGroups > k_pNumBankGroups) {
      output->output("%s, Warning!: Number of address map bankGroups is larger "
                     "than numBankGroupsPerRank.\n"
                     "Some BankGroups will be unused\n\n",
                     getName().c_str());
    }
    if (l_aNumBankGroups <
        k_pNumBankGroups) { // some addresses have nowhere to go
      output->fatal(CALL_INFO, -1,
                    "%s, Error!: Number of address map bankGroups is smaller "
                    "than numBankGroupsPerRank. Aborting!\n",
                    getName().c_str());
    }
  } // else found in map
  ...
  if (l_it == m_structureSizes.end()) { // if not found
    if (k_pNumBanks > 1) {
      output->fatal(CALL_INFO, -1,
                    "%s, Number of Banks (%u) is greater than 1, but no "
    ...
    if (l_aNumBanks > k_pNumBanks) {
      output->output("%s, Warning!: Number of address map Banks is larger than "
                     "numBanksPerBankGroup.\n"
                     "Some Banks will be unused\n\n",
                     getName().c_str());
    }
    if (l_aNumBanks < k_pNumBanks) { // some addresses have nowhere to go
      output->fatal(CALL_INFO, -1,
                    "%s, Error!: Number of address map Banks is smaller than "
                    "numBanksPerBankGroup. Aborting!\n",
                    getName().c_str());
    }
  } // else found in map
  ...
  if (l_it == m_structureSizes.end()) { // if not found
    if (k_pNumRows > 1) {
      output->fatal(CALL_INFO, -1,
                    "%s, Number of Rows (%u) is greater than 1, but no "
    ...
    if (l_aNumRows > k_pNumRows) {
      output->output("%s, Warning!: Number of address map Rows is larger than "
                     "numRowsPerBank.\n"
    ...
    if (l_aNumRows < k_pNumRows) { // some addresses have nowhere to go
      output->fatal(CALL_INFO, -1,
                    "%s, Error!: Number of address map Rows is smaller than "
                    "numRowsPerBank. Aborting!\n",
                    getName().c_str());
    }
  } // else found in map
  ...
  if (l_it == m_structureSizes.end()) { // if not found
    if (k_pNumCols > 1) {
      output->fatal(
          CALL_INFO, -1,
          "%s, Number of Cols (%u) is greater than 1, but no "
    ...
    if (l_aNumCols > k_pNumCols) {
      output->output("%s, Warning!: Number of address map Cols is larger than "
                     "numColsPerBank.\n"
    ...
    if (l_aNumCols < k_pNumCols) { // some addresses have nowhere to go
      output->fatal(CALL_INFO, -1,
                    "%s, Error!: Number of address map Cols is smaller than "
                    "numColsPerBank. Aborting!\n",
                    getName().c_str());
    }
  } // else found in map
  ...
  if (l_it == m_structureSizes.end()) { // if not found
    if (k_pBurstSize > 1) {
      output->fatal(
          CALL_INFO, -1,
          "%s, Burst size (%u) is greater than 1, but no "
    ...
    if (l_aNumCachelines != k_pBurstSize) {
      output->fatal(
          CALL_INFO, -1,
          "%s, Error!: Number of address map Cachelines is not equal to "
          "numBytesPerTransaction.\n"
          "Make sure that the address map cachelines (h) are equal to "
          "numBytesPerTransaction (i.e. 2**h == numBytesPerTransaction!",
          getName().c_str());
    }
  } // else found in map
...
void c_AddressHasher::fillHashedAddress(c_HashedAddress *x_hashAddr,
                                        const ulong x_address) {
  ulong l_cur = 0;
  ulong l_cnt = 0;
  ...
  unsigned l_bankId =
      x_hashAddr->getBank() + x_hashAddr->getBankGroup() * k_pNumBanks +
      x_hashAddr->getRank() * k_pNumBanks * k_pNumBankGroups +
      x_hashAddr->getPChannel() * k_pNumBanks * k_pNumBankGroups * k_pNumRanks +
      x_hashAddr->getChannel() * k_pNumPseudoChannels * k_pNumBanks *
          k_pNumBankGroups * k_pNumRanks;

  unsigned l_rankId =
      x_hashAddr->getRank() + x_hashAddr->getPChannel() * k_pNumRanks +
...
ulong c_AddressHasher::getAddressForBankId(const unsigned x_bankId) {
  ...
  unsigned l_chanSize =
      k_pNumBanks * k_pNumBankGroups * k_pNumRanks * k_pNumPseudoChannels;
  ...
  output->output("Final %u %u %u %u %u\n", l_chan, l_pchan, l_rank, l_bankgroup,
                 l_bank);
  ...
  // cout << "Returning address 0x" << std::hex << l_address << std::dec <<
  // endl;
...
void c_AddressHasher::parsePattern(string *x_inStr,
                                   std::pair<string, uint> *x_outPair) {
  ...
  while (!l_matched) {
    if (isdigit(*l_sIter)) {
      if (l_sizeMatched) {
        output->fatal(CALL_INFO, -1,
                      "%s, Weird parsing detected!\nParsing error at %c in "
                      "address map string %s]\n",
                      getName().c_str(), (*l_sIter), (*x_inStr).c_str());
      }
      l_sizeStr = *l_sIter + l_sizeStr;
    } else if (isalpha(*l_sIter)) {
      if (!(*l_sIter == 'r' || *l_sIter == 'l' || *l_sIter == 'R' ||
            *l_sIter == 'B' || *l_sIter == 'b' || *l_sIter == 'C' ||
            *l_sIter == 'h' || *l_sIter == 'c' || *l_sIter == 'x')) {
        output->fatal(CALL_INFO, -1,
                      "%s, Parsing error at %c in address map string %s\n",
                      getName().c_str(), (*l_sIter), (*x_inStr).c_str());
      }
      ...
      x_inStr->erase(next(l_sIter).base(),
                     x_inStr->end()); // remove the matched portion
      // cout << "Returning " << x_outPair->first << " " << x_outPair->second <<
      // endl;
      break;
    } else if (*l_sIter == ':') {
      l_sizeMatched = true;
