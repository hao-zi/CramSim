...
c_MemhBridge::c_MemhBridge(ComponentId_t x_id, Params &x_params)
  ...
  output = new SST::Output("CramSim.memhbridge[@f:@l:@p] ", verbosity, 0,
                           SST::Output::STDOUT);

  // set up txn trace output
  k_printTxnTrace =
      (uint32_t)x_params.find<uint32_t>("boolPrintTxnTrace", 0, l_found);

  k_txnTraceFileName =
      (std::string)x_params.find<std::string>("strTxnTraceFile", "-", l_found);
  ...
  if (k_printTxnTrace) {
    if (k_txnTraceFileName.compare("-") == 0) { // set output to std::cout
      ...
      std::cout << "Setting txn trace output to " << k_txnTraceFileName
                << std::endl;
      m_txnTraceOFStream.open(k_txnTraceFileName);
      if (m_txnTraceOFStream) {
        m_txnTraceStreamBuf = m_txnTraceOFStream.rdbuf();
      } else {
        std::cerr << "Failed to open txn trace output file "
                  << k_txnTraceFileName << ", redirecting to stdout";
        m_txnTraceStreamBuf = std::cout.rdbuf();
      }
    }
...
void c_MemhBridge::createTxn() {
  ...
  while ((e = m_linkCPU->recv())) {
    ...
    if (event->getIsWrite())
      mTxn = new c_Transaction(event->getReqId(), e_TransactionType::WRITE,
                               addr, 1);
    else
      mTxn = new c_Transaction(event->getReqId(), e_TransactionType::READ, addr,
                               1);
...
bool c_MemhBridge::readResponse() {
  if (m_txnResQ.size() > 0) {
    ...
    MemRespEvent *event =
        new MemRespEvent(l_txn->getSeqNum(), l_txn->getAddress(), 0);
...
void c_MemhBridge::printTxn(bool x_isWrite, uint64_t x_addr) {
  ...
  (*m_txnTraceStream) << "0x" << std::hex << x_addr << " " << l_txnType << " "
                      << std::dec << l_currentCycle << std::endl;
}
