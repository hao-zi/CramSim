...
public:
  SST_ELI_REGISTER_COMPONENT(c_Controller, "CramSim", "c_Controller",
                             SST_ELI_ELEMENT_VERSION(1, 0, 0),
                             "Memory Controller",
                             COMPONENT_CATEGORY_UNCATEGORIZED)

  SST_ELI_DOCUMENT_PARAMS({"verbose", "Output verbosity", "0"},
                          {"strControllerClockFrequency",
                           "Controller clock frequency, with units", "1GHz"})

  SST_ELI_DOCUMENT_PORTS({"txngenLink",
                          "link to txn generator / txn dispatcher",
                          {"c_txnGenReqEvent"}},
                         {"memLink", "link to memory", {"c_DeviceResEvent"}}, )

  SST_ELI_DOCUMENT_SUBCOMPONENT_SLOTS(
      {"AddrMapper", "address hasher. Default: CramSim.c_AddressHasher",
       "SST::CramSim::c_AddressHasher"},
      {"TxnScheduler", "Transaction Scheduler. Default: CramSim.c_TxnScheduler",
       "SST::CramSim::c_TxnScheduler"},
      {"TxnConverter", "Transaction Converter. Default: CramSim.c_TxnConverter",
       "SST::CramSim::c_TxnConverter"},
      {"CmdScheduler", "Command Scheduler. Default: CramSim.c_CmdScheduler",
       "SST::CramSim::c_CmdScheduler"},
      {"DeviceDriver", "device driver. Default: CramSim.c_DeviceDriver",
