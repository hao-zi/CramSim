...
c_TxnGenBase::c_TxnGenBase(ComponentId_t x_id, Params &x_params)
  ...
  output = new SST::Output("CramSim.TxnGen[@f:@l:@p] ", verbosity, 0,
                           SST::Output::STDOUT);
  ...
  if (!l_found) {
    std::cout << "TxnGen:: numTxnPerCycle is missing... exiting" << std::endl;
    exit(-1);
  }

  k_maxOutstandingReqs =
      x_params.find<std::uint32_t>("maxOutstandingReqs", 0, l_found);
  if (!l_found) {
    std::cout << "TxnGen:: maxOutstandingReqs is missing... No limit to the "
  ...
  if (!l_found) {
    std::cout << "TxnGen:: maxTxns is missing... No limit to the maximum "
                 "number of transactions"
              << std::endl;
    // exit(-1);
  }

  uint32_t k_numBytesPerTransaction =
      x_params.find<std::uint32_t>("numBytesPerTransaction", 32, l_found);
  if (!l_found) {
  ...
  std::string l_controllerClockFreqStr =
      (std::string)x_params.find<std::string>("strControllerClockFrequency",
                                              "1GHz", l_found);

  // set our clock
  registerClock(l_controllerClockFreqStr, new Clock::Handler<c_TxnGenBase>(
                                              this, &c_TxnGenBase::clockTic));
...
c_TxnGenBase::c_TxnGenBase() : Component(-1) {
...
void c_TxnGenBase::finish() {
  ...
  std::cout << "Cycles Per Transaction (CPT) = " << std::dec
            << static_cast<double>(m_simCycle) /
                   static_cast<double>(m_resReadCount + m_resWriteCount)
            << std::endl;
...
bool c_TxnGenBase::clockTic(Cycle_t) {
  ...
  for (int i = 0; i < k_numTxnPerCycle; i++) {
    if (k_maxOutstandingReqs == 0 ||
        m_numOutstandingReqs < k_maxOutstandingReqs) {
...
void c_TxnGenBase::handleResEvent(SST::Event *ev) {
  ...
  if (l_txnResEventPtr) {
    ...
    if (l_txn->getTransactionMnemonic() == e_TransactionType::READ) {
    ...
    output->verbose(CALL_INFO, 1, 0,
                    "[cycle:%lld] addr: 0x%lx isRead:%d seqNum:%lld "
                    "birthTime:%lld latency:%lld \n",
                    l_currentCycle, l_txn->getAddress(), l_txn->isRead(),
                    l_seqnum, m_outstandingReqs[l_seqnum], l_latency);
#endif
    ...
    std::cout << std::endl
              << std::endl
              << "TxnGen:: " << __PRETTY_FUNCTION__
              << " ERROR:: Bad Event Type!" << std::endl;
  }
}

bool c_TxnGenBase::sendRequest() {
  assert(k_maxOutstandingReqs == 0 ||
         m_numOutstandingReqs <= k_maxOutstandingReqs);
  if (!m_txnReqQ.empty()) {
    ...
    if (m_txnReqQ.front().second > l_cycle) {
      // std::cout << __PRETTY_FUNCTION__ << ": Interval timer for front txn is
      // not done"
      // << std::endl;
...
#ifdef __SST_DEBUG_OUTPUT__
    output->verbose(CALL_INFO, 1, 0,
                    "[cycle:%lld] addr: 0x%x isRead:%d seqNum:%lld\n", l_cycle,
                    l_txn->getAddress(), l_txn->isRead(), l_txn->getSeqNum());
#endif

    m_outstandingReqs.insert(
        std::pair<uint64_t, uint64_t>(l_txn->getSeqNum(), l_cycle));
    return true;
  } else
    return false;
}
...
c_TxnGen::c_TxnGen(ComponentId_t x_id, Params &x_params)
  ...
  k_readWriteTxnRatio = x_params.find<float>("readWriteRatio", 0.5, l_found);
  if (!l_found) {
    std::cout << "TxnGen:: readWriteRatio value is missing... it will be 0.5 "
                 "(default)"
              << std::endl;
    // exit(-1);
  }
  ...
  if (!l_found) {
    std::cout << "TxnGen:: mode is missing... exiting" << std::endl;
    exit(-1);
  } else {
    if (l_mode == "rand")
  ...
  std::string l_randSeedStr =
      x_params.find<std::string>("randomSeed", "0", l_found);
...
void c_TxnGen::createTxn() {
  ...
  while (m_txnReqQ.size() < k_numTxnPerCycle) {
    ...
    std::pair<c_Transaction *, uint64_t> l_entry =
        std::make_pair(mTxn, l_cycle);
