...
c_TxnConverter::c_TxnConverter(SST::ComponentId_t id, SST::Params &x_params,
                               Output *out, unsigned banks,
                               c_CmdScheduler *scheduler)
    : SubComponent(id), output(out), m_cmdScheduler(scheduler) {
...
void c_TxnConverter::build(SST::Params &x_params, unsigned l_bankNum) {
  ...
  k_relCommandWidth =
      (uint32_t)x_params.find<uint32_t>("relCommandWidth", 1, l_found);
  if (!l_found) {
  ...
  std::string l_bankPolicy =
      (std::string)x_params.find<std::string>("bankPolicy", "CLOSE", l_found);
  if (!l_found) {
    std::cout << "bankPolicy value is missing... it will be \"close-page "
                 "policy\" (Default)"
              << std::endl;
  }
  if (l_bankPolicy == "CLOSE")
    ...
    k_bankPolicy = 2;
    k_bankCloseTime =
        (SimTime_t)x_params.find<SimTime_t>("bankCloseTime", 1000, l_found);
    if (!l_found) {
      std::cout << "bank policy is set to pseudo open policy, but "
                   "bankCloseTime value is missing... exiting"
                << std::endl;
      exit(-1);
    }
    if (k_bankPolicy == 2) {
...
void c_TxnConverter::run(SimTime_t simCycle) {
  ...
  for (std::deque<c_Transaction *>::iterator l_it = m_inputQ.begin();
       l_it != m_inputQ.end();) {
    c_Transaction *l_reqTxn = *l_it;

    unsigned l_numToken =
        m_cmdScheduler->getToken(l_reqTxn->getHashedAddress());
    if (l_numToken >= 3) {
...
void c_TxnConverter::getPreCommands(std::vector<c_BankCommand *> &x_commandVec,
                                    c_Transaction *x_txn, ulong x_nAddr) {
  ...
  // close policy
  if (k_bankPolicy == 0) {
    x_commandVec.push_back(new c_BankCommand(
        m_cmdSeqNum++, e_BankCommandType::ACT, x_nAddr, l_hashedAddr));
    ...
    if (l_bankinfo->isRowOpen()) {
      ...
      if (l_bankinfo->getOpenRowNum() != l_row) {
        ...
        x_commandVec.push_back(new c_BankCommand(
            m_cmdSeqNum++, e_BankCommandType::PRE, x_nAddr, l_hashedAddr));
        // open row
        x_commandVec.push_back(new c_BankCommand(
            m_cmdSeqNum++, e_BankCommandType::ACT, x_nAddr, l_hashedAddr));
...
void c_TxnConverter::getPostCommands(std::vector<c_BankCommand *> &x_commandVec,
                                     c_Transaction *x_txn, ulong x_nAddr) {
